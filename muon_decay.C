...
void muon_decay()
{
  ...
  for (int ievt=0; ievt<1; ievt++)
  {
	...
	{ // pion decay
	  ...
	  { // down-going only
            float E_mu = Muon.E();
	    float muon_d_t = r -> Exp(Tau_mu);
	    float l_muon_d = Get_D_L(E_mu,M_mu,muon_d_t);

	    float det_x = 0; // detector location, in km
	    float det_y = 0;
	    float det_z = 0; // i.e. on ground

	    float l_muon_v_to_det = pow(pow(mu_vx_x-det_x,2)+
					pow(mu_vx_y-det_y,2)+
					pow(mu_vx_z-det_z,2),0.5);
	    // just assume vertical distance to the ground
	    // not taking into account angle effects !

	    if (l_muon_d > l_muon_v_to_det)
	    {
